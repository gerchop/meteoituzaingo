...
function listaNatural(items) { return items.length < 2 ? items[0] || "" : `${items.slice(0, -1).join(", ")} y ${items.at(-1)}`; }
function formatoEvidenciaAviso(periods) {
  const labels = { dawn: "madrugada", morning: "mañana", afternoon: "tarde", night: "noche" };
  return periods.map((period) => {
    const parts = (period.dayParts || []).map((part) => labels[part]).filter(Boolean);
    const description = parts.length === 4 ? "todo el día" : listaNatural(parts);
    return description ? `${fechaAvisoLocal(period.date)} — ${description}` : "";
  }).filter(Boolean).join(" / ");
}
function textoTemporalAviso(advisory) {
  if (advisory.temporalPrecision === "daily" && advisory.targetLocalDate) return `Jornada prevista: ${fechaAvisoLocal(advisory.targetLocalDate)}`;
  if (advisory.temporalPrecision === "hourly" && Array.isArray(advisory.evidencePeriods)) {
    const evidence = formatoEvidenciaAviso(advisory.evidencePeriods);
    if (evidence) return `Período previsto: ${evidence}`;
  }
...
async function obtenerPronosticoMeteored(tipo) {
  ...
  if (!respuesta.ok) { const error = new Error(cuerpo?.error || `Meteored respondió ${respuesta.status}`); error.forecastCacheState = cuerpo?.cache?.state; throw error; }
...
function fuenteCachePronostico(tipo, cache) {
  ...
  if (cache?.cache?.state === "STALE_USABLE" && cache.cache.updatedAt) fuente.textContent = `Fuente: Meteored · Pronóstico sin actualización reciente. Última actualización: ${DateTime.formatTime(new Date(cache.cache.updatedAt))}.`;
