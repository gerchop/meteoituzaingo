...
function socialScript() { return `(()=>{let csrf='';const $=id=>document.getElementById(id),api=async(path,options={})=>{const r=await fetch(path,{credentials:'same-origin',headers:{'Content-Type':'application/json',...(options.mutation?{'X-CSRF-Token':csrf}:{})},...options});const b=await r.json();if(!r.ok||!b.ok)throw new Error(b.error||'Error');return b.data},copy=async text=>{try{await navigator.clipboard.writeText(text)}catch{const t=document.createElement('textarea');t.value=text;document.body.append(t);t.select();document.execCommand('copy');t.remove()}};function show(data){$('app').style.display='block';$('login').style.display='none';if(!data){$('title').textContent='No hay un pronóstico generado para hoy.';$('text').value='';$('parts').textContent='Usá Regenerar pronóstico para intentar crearlo.';return}$('title').textContent='Pronóstico para '+data.date;$('meta').textContent=data.status+' · '+data.source+' · generado '+new Date(data.generatedAt).toLocaleString('es-AR');$('text').value=data.finalText||data.originalText||'';$('parts').replaceChildren(...data.parts.map((part,i)=>{const d=document.createElement('div');d.className='part';const p=document.createElement('pre');p.textContent=(i+1)+'/'+data.parts.length+'\\n'+part;const b=document.createElement('button');b.className='secondary';b.textContent='Copiar publicación';b.onclick=()=>copy(part);d.append(p,b);return d}))}async function load(){try{show(await api('/api/admin/social-forecast/today'));const history=await api('/api/admin/social-forecast/history');$('history').replaceChildren(...history.map(x=>{const b=document.createElement('button');b.className='secondary';b.textContent=x.date+' — '+x.status+' — '+x.partsCount+' publicación(es)';b.onclick=()=>show(x);return b}))}catch(e){$('loginStatus').textContent='La sesión venció. Ingresá nuevamente.';$('app').style.display='none';$('login').style.display='block'}}$('loginForm').onsubmit=async e=>{e.preventDefault();try{const result=await api('/api/admin/login',{method:'POST',body:JSON.stringify({password:$('password').value})});csrf=result.csrf;$('password').value='';await load()}catch(err){$('loginStatus').textContent=err.message}};$('save').onclick=async()=>{try{show(await api('/api/admin/social-forecast',{method:'PUT',mutation:true,body:JSON.stringify({text:$('text').value})}));$('status').textContent='Cambios guardados.'}catch(e){$('status').textContent=e.message}};$('regenerate').onclick=async()=>{if(!confirm('Esto reemplazará la edición actual. ¿Continuar?'))return;try{show(await api('/api/admin/social-forecast/regenerate',{method:'POST',mutation:true}));$('status').textContent='Pronóstico regenerado.'}catch(e){$('status').textContent=e.message}};$('logout').onclick=async()=>{await api('/api/admin/logout',{method:'POST'});location.reload()}})()`; }
